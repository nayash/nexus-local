...
from src.tools.tabular import load_tabular_dataframe, generate_tabular_analysis_code, execute_tabular_analysis
...
def analyze_tabular_file_tool(file_path: str, user_query: str):
    ...
    output_parts = []
    if result.stdout:
        output_parts.append(f"Sandbox output:\n{result.stdout.strip()}")
    if result.stderr:
        output_parts.append(f"Sandbox errors:\n{result.stderr.strip()}")
    if result.timed_out:
        output_parts.append("Sandbox execution timed out.")
    elif result.exit_code != 0 and not result.stderr:
        output_parts.append(f"Sandbox exited with code {result.exit_code}.")
    if not output_parts:
        output_parts.append("The sandbox completed with no output.")
